...
% files listed above and the 'lib' folder. If it does not, right-click the 'ex2' 
% folder and select 'Open' before proceding or see the instructions in |README.mlx| 
% for more details.

clear
cd /Users/firstsingularity/Desktop/machine-learning-ex/ex2
dir
% Before you begin
...
% 
% 
% To help you get more familiar with plotting, we have left |plotData.m| empty 
% so you can try to implement it yourself. However, this is an optional (ungraded) 
% exercise. We also provide our implementation below so you can copy it or refer 
% to it. If you choose to copy our example, make sure you learn what each of its 
% commands is doing by consulting the MATLAB documentation.
%%
% 
...
%
%% 
% Once you have added your own or the above code to |plotData.m|, run the code 
% in this section to call the |plotData| function.
...
% values by calling |sigmoid(x)| in the code section below. For large positive 
...
sigmoid([1 2 3 4 5 6 90 ; 90 0 -299 9 7 55 -89])
%% 
...
% the $j$th element (for $j = 0,\; 1,\ldots, n)$ is defined as follows:
% 
...
% different definitions of $h_{\theta}(x)$. Once you are done, run the code sections 
% below to set up your data and call your |costFunction| using two sets of initial 
% values of $\theta$. 
% Initialize the data
...
% You should see that the cost is about 0.693 and gradients of about -0.1000,  
% -12.0092, and -11.2628 for all-zero $\theta$, and that the cost is  0.219 with 
% gradients of 0.043, 2.566, 2.640 for non-zero $\theta$.
...
% |fminunc|. 
% 
% MATLAB's |fminunc| is an optimization solver that finds the minimum of an 
% unconstrained* function. For logistic regression, you want to optimize the cost 
% function $J(\theta)$ with parameters. Concretely, you are going to use |fminunc| 
% to find the best parameters $\theta$ for the logistic regression cost function, 
% given a fixed dataset (of $X$ and $y$ values). You will pass to |fminunc| the 
% following inputs:
%% 
...
% for example, constraints that bound the possible values_ $\theta$ _can take 
% (e.g._ $\theta<1$_). Logistic regression does not have such constraints since_ 
% $\theta$ _is allowed to take any real value._
% 
% 
% 
...
% $\theta$.
% 
% After you have completed the code in |predict.m|, the code below will proceed 
% to report the training accuracy of your classifier by computing the percentage 
% of examples it got correct. 
...
hold off;
%% 
% Figure 3 shows that our dataset cannot be separated into positive and negative 
% examples by a straight-line through the plot. Therefore, a straightforward application 
% of logistic regression will not perform well on this dataset since logistic 
...
% 
% $$\textrm{mapFeature}\left(x\right)=\left\lbrack \begin{array}{c}1\\x_1 \\x_2 
% \\x_1^2 \\x_1 x_2 \\x_2^2 \\x_1^3 \\\vdots \;\\x_1 x_2^5 \\x_2^6 \end{array}\right\rbrack$$
% 
% As a result of this mapping, our vector of two features (the scores on two 
% QA tests) has been transformed into a 28-dimensional vector. A logistic regression 
% classifier trained on this higher-dimension feature vector will have a more 
% complex decision boundary and will appear nonlinear when drawn in our 2-dimensional 
% plot. 
% 
...
% 
% Note that you should not regularize the parameter $\theta_0$. In MATLAB, recall 
% that indexing starts from 1, hence, you should not be regularizing the |theta(1)| 
% parameter (which corresponds to $\theta_0$) in the code. The gradient of the 
% cost function is a vector where the $j\mathrm{th}$ element is defined as follows:
% 
% $$\frac{\partial J(\theta)}{\partial \theta_j} = \frac{1}{m}\sum_{i=1}^m{\left( 
...
% that the cost is about 0.693 for all-zero $\theta$ and 3.16 for the non-zero 
% $\theta$ test case.
...
% 
% 
% This is not a good decision boundary: for example, it predicts that a point 
% at $x = (0.25, 1.5)$ is accepted $(y = 1)$, which seems to be an incorrect decision 
% given the training set. With a larger $\lambda$, you should see a plot that 
% shows an simpler decision boundary which still separates the positives and negatives 
% fairly well. However, if $\lambda$ is set to too high a value, you will not 
% get a good fit and the decision boundary will not follow the data so well, thus 
% underfitting the data (Figure 6).
% 
% 
% 
